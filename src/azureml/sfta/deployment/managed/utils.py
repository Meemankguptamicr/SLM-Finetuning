...
def deploy_base_model_to_online_endpoint(ml_client, registry_name, model_name, endpoint_name, deployment_name, instance_type, instance_count=1, auth_mode="key"):
...
def deploy_custom_model_to_online_endpoint(ml_client, endpoint_name, model_name, model, env, code_path, instance_type, instance_count=1, auth_mode="key"):  
    try:
        endpoint = ml_client.online_endpoints.get(endpoint_name)
...
def deploy_base_to_managed_compute(ml_client, sample_input_data, registry_name, base_model_name, endpoint_name, deployment_name, instance_type, instance_count=1, auth_mode="key"):
    deploy_base_model_to_online_endpoint(ml_client, registry_name, base_model_name, endpoint_name, deployment_name, instance_type, instance_count, auth_mode)
    test_online_endpoint_deployment(ml_client, endpoint_name, sample_input_data)
    print("Finished Base Model Deployment")


def deploy_finetuned_to_managed_compute(ml_client, sample_input_data, model_dir, model_name, endpoint_name, vm_type, instance_type, instance_count=1, auth_mode="key"):
    model = Model(
        path=model_dir,
        type="custom_model",
        name=model_name,
